...
('COLLABORATEUR', 'Utilisateur qui soumet et suit les demandes'),
('RESPONSABLE', 'Utilisateur qui traite et valide les demandes'),
('ADMINISTRATEUR', 'Utilisateur qui administre le système'),
('AGENT_ACCUEIL', 'Utilisateur qui gère l accueil et les visites');
SELECT * FROM role;
...
INSERT INTO utilisateur(nom, prenom, email, mot_de_passe, telephone, role_id, departement_id) VALUES
('Dami', 'Rayane', 'rayane@portail.ma', '123456', '0600000000', 1, 2),
('Test', 'Responsable', 'responsable@portail.ma', '123456', '0600000001', 2, 2),
('Test', 'Administrateur', 'admin@portail.ma', '123456', '0600000002', 3, 3),
('Test', 'Accueil', 'accueil@portail.ma', '123456', '0600000003', 4, 3);
...
('Attestation', 'Demande d''une attestation administrative'),
('Congé', 'Demande de congé'),
('Autorisation', 'Demande d''autorisation'),
('Document administratif', 'Demande de document administratif');
SELECT * FROM type_demande;


-- représente une demande soumise par un collaborateur.
INSERT INTO demande (date_soumission, motif, statut, collaborateur_id, type_demande_id) VALUES
(NOW(), 'Demande d''attestation administrative', 'EN_ATTENTE', 1, 1);
SELECT
    d.id,
    d.date_soumission,
    d.motif,
...
FROM demande d
JOIN utilisateur u
    ON d.collaborateur_id = u.id
JOIN type_demande td
    ON d.type_demande_id = td.id;


--  contient les fichiers associés à une demande
INSERT INTO piece_jointe (nom_fichier, url_fichier, type_fichier, taille, demande_id) VALUES
('attestation.pdf', '/uploads/attestation.pdf', 'application/pdf', 250000, 1);
SELECT
    p.id,
    p.nom_fichier,
    p.type_fichier,
...
FROM piece_jointe p
JOIN demande d
    ON p.demande_id = d.id;

-- gère les différentes étapes de validation d'une demande.    
INSERT INTO validation (niveau, decision, commentaire, demande_id, responsable_id) VALUES
(1, 'EN_ATTENTE', NULL, 1, 2);
SELECT
    v.id,
    v.niveau,
    v.decision,
...
INSERT INTO visiteur (nom, prenom, email, telephone, societe) VALUES
('Alami', 'Youssef', 'youssef.alami@email.com', '0612345678', 'ABC Consulting');
SELECT * FROM visiteur;


-- représente un rendez-vous entre un collaborateur et un visiteur.
INSERT INTO rendez_vous(date_rendez_vous, heure_rendez_vous, motif, statut, collaborateur_id, visiteur_id) VALUES
('2026-08-25', '10:00:00', 'Réunion professionnelle', 'PLANIFIE', 1, 1);
SELECT
    r.id,
    r.date_rendez_vous,
    r.heure_rendez_vous,
...
FROM rendez_vous r
JOIN utilisateur u
    ON r.collaborateur_id = u.id
JOIN visiteur v
    ON r.visiteur_id = v.id;

-- représente le badge avec le QR Code associé à un rendez-vous.
INSERT INTO badge(qr_code, date_expiration, statut, rendez_vous_id)VALUES
('QR-RDV-000001', '2026-08-25 23:59:59', 'VALIDE', 1);
SELECT
    b.id,
    b.qr_code,
    b.date_generation,
...
FROM badge b
JOIN rendez_vous r
    ON b.rendez_vous_id = r.id
JOIN visiteur v
    ON r.visiteur_id = v.id;
    
    
-- enregistre l'arrivée et le départ d'un visiteur.    
INSERT INTO visite(date_entree, date_sortie, statut, rendez_vous_id, agent_accueil_id)VALUES
(NULL, NULL, 'PREVUE', 1, 4);    
SELECT
    vi.id,
    vi.date_entree,
    vi.date_sortie,
...
FROM visite vi
JOIN utilisateur u
    ON vi.agent_accueil_id = u.id;


-- informe les utilisateurs des événements importants du système.
INSERT INTO notification (message, type, est_lue, utilisateur_id, demande_id)
VALUES(
...
FROM notification n
JOIN utilisateur u
    ON n.utilisateur_id = u.id;
    

-- conserve l'historique des actions effectuées dans le système.
INSERT INTO log(action, adresse_ip, utilisateur_id)
VALUES(
...
WHERE TABLE_SCHEMA = 'portail_services'
