...
const Notification = {
    ...
    async notifyVisitor(rendez_vous_id, message, type, expediteur_id = null) {
        ...
        return this.notifyUser(rows[0].utilisateur_id, message, type, null, rendez_vous_id, expediteur_id);
    },
    ...
    async update(id, data) {
        const { message, type, est_lue = 0 } = data;

        await db.query(
            `UPDATE notification
             SET message = ?, type = ?, est_lue = ?
             WHERE id = ?`,
    ...
    async getAll(auth) {
        const where = auth?.role === "ADMINISTRATEUR" ? "" : "WHERE n.utilisateur_id = ?";
        ...
        const [rows] = await db.query(`
            SELECT n.id, n.message, n.type, n.date_envoi, n.est_lue,
                   ...
                   role_exp.nom AS expediteur_role
            FROM notification n
            LEFT JOIN utilisateur exp ON exp.id = n.expediteur_id
            LEFT JOIN role role_exp ON role_exp.id = exp.role_id
            ${where}
            ORDER BY n.id DESC
        `, params);
